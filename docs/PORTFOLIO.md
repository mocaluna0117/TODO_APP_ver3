...
> 期限通知・画像/PDF添付・タグ分類・優先度・繰り返し・バックアップまで備えた、実用レベルのタスク管理モバイルアプリ。要件定義からデータモデル設計、UI/UX、機能実装、実機デプロイまでを個人で担当した。
...
- ゴミ箱：削除は論理削除として扱い、復元・完全削除・7日での自動削除に対応
- 5 つのカテゴリタブで整理
  - **やること** … 通常のタスク
...
- **繰り返し**：プリセット（毎日 / 毎週○曜日 / 平日 / 毎月n日 / 毎月第n○曜日 / 毎年）＋カスタム設定（間隔・曜日の複数指定・月内の位置・終了条件）。完了すると次回の期限へ自動で進み、終了条件に達すると完了扱いになる
- **並び順**：期限の昇順 / 降順

### 画像・PDF添付

- カメラ撮影・ギャラリー選択でタスクに画像を複数添付
- PDF をタスクに複数添付。実体は Firebase Storage、タスク側は URL とファイル名のみを保持し、Firestore の1ドキュメント1MB制限を避けている
